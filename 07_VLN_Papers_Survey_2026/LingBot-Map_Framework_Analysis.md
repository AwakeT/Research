...
### 0.1 LingBot-Map 解决什么问题

LingBot-Map 解决的核心问题是：**如何在流式（在线）设定下，用有限且近乎恒定的计算资源管理不断增长的视觉观测历史，同时保持长序列上的全局 3D 几何一致性。**

更具体地说，它是传统 SLAM（同步定位与建图）系统的**端到端 Transformer 替代方案**。传统 SLAM 的流水线由多个手工设计的模块串联而成——特征提取、特征匹配、PnP 求解、束调整（Bundle Adjustment）、回环检测——每个模块都有自己的假设、阈值和失败模式。LingBot-Map 用一个统一的前馈 Transformer 取代了整个流水线，由网络自动学习"如何从视频流中提取几何信息并保持全局一致性"。

### 0.2 目标场景

用户拿着手机（或机器人带着相机）在场景中移动，视频帧**逐帧到达**，系统需要**实时**输出三类结果：

| 输出 | 含义 | 用途 |
|---|---|---|
| **相机位姿** | 当前帧的相机在三维空间中的位置和朝向（6DoF） | 定位——"我在哪" |
| **深度图** | 每个像素到相机的距离 | 感知——"周围多远" |
| **3D 点云** | 将所有帧的深度反投影拼成完整的三维世界坐标 | 建图——"世界长什么样" |

关键约束是**流式/在线**——不能看到未来帧，必须来一帧处理一帧，且要求接近实时（~20 FPS）。

典型应用场景包括：

- **手持设备 3D 扫描**：手机拍摄建筑/房间，实时生成 3D 模型
- **机器人实时建图**：移动机器人在未知环境中边走边建图
- **AR/VR 场景理解**：头戴设备需要实时理解三维空间结构
- **自动驾驶定位**：车载相机在城市场景中的长距离定位（VO 模式）

### 0.3 本质矛盾：历史信息管理

单帧 3D 重建无法确定绝对尺度和全局一致性（从一张图无法判断是 1 米远的小盒子还是 100 米远的大楼），所以模型必须依赖历史帧信息。但历史信息的管理存在**根本矛盾**：

- **信息太少**（激进压缩）→ 丧失全局一致性，长距离累积漂移严重
- **信息太多**（全量保留）→ 显存爆炸，远距离帧的冗余 patch 反而引入噪声，性能下降
- **手工设计**（传统 SLAM 启发式）→ 依赖人工阈值，不可微分，无法端到端优化

LingBot-Map 的解法是：**不同历史帧承担不同功能，应该保留不同粒度的信息**——远距离帧只需要保留"在哪里看到了什么"的摘要（6 个 token），近距离帧需要保留完整的视觉细节（~500 个 token），锚定帧需要永久保留以维持全局坐标系。这就是 GCA 三级上下文的设计哲学。

### 0.4 已有方案为什么不够

论文发表前有三类已有方案，每类都有致命缺陷：

| 方案 | 代表工作 | 做法 | 致命问题 |
|---|---|---|---|
| **循环压缩** | CUT3R | 把所有历史压到一个固定大小的隐状态 | 压缩太激进，几何先验被遗忘；序列从 320 帧扩展到 3840 帧时 ATE 恶化 +78%（18.16 → 32.47） |
| **全量因果缓存** | StreamVGGT, Stream3R | 保留所有历史帧的 K/V 缓存 | 显存线性增长（10000 帧 ≈ 500 万 token）；远距离帧的 patch token 与当前帧视觉重叠极小，冗余信息反而引入噪声（ATE 6.60 vs GCA 的 5.98） |
| **SLAM 混合** | VGGT-SLAM, MASt3R-SLAM | 学习模型 + 传统优化拼接 | 依赖手工启发式（关键帧选择阈值、回环检测参数），不可微分，实时性受限 |

LingBot-Map 的 GCA 用一个统一的、可微分的注意力机制同时解决了这三个问题：精度超过全量缓存（去除了冗余噪声）、效率接近滑窗（每帧仅增长 6 个 token）、不依赖任何手工启发式（端到端学习）。

### 0.5 效果总结

| 维度 | 指标 | 说明 |
|---|---|---|
| **精度** | ATE 6.42（在线方法中最优，超越最强离线方法 DA3 的 12.87） | 在 Oxford Spires 320 帧上 |
| **长序列鲁棒性** | 320→3840 帧仅退化 10.7%（竞品 CUT3R 退化 78%） | 验证三级上下文的长程一致性 |
| **效率** | ~20 FPS，每帧上下文增长仅 6 token（全量方案 ~500 token/帧） | ~80 倍压缩，且精度反而更高 |
| **3D 重建** | ETH3D F1 = 98.98（Wint3R 77.28，Stream3R 72.87） | 重建质量大幅领先 |
