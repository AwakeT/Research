...
Qwen3.5 小模型（0.8B/2B/4B/9B）thinking 默认关闭，大模型（27B+）默认开启。

**采样参数也不同**：Thinking 模式使用高温度（1.0）鼓励推理多样性，No-thinking 使用低温度（0.7）确保输出确定性。

### 3.2 Thinking 模式的行为差异实测
...
### 3.3 关键发现：Thinking 的价值因任务性质分化

| 任务性质 | 推荐模式 | 原因 |
|---------|---------|------|
| 感知密集型（检测、定位、枚举） | **No-Thinking** | thinking 产生"过度推理"，干扰快速感知输出 |
| 推理密集型（空间推理、搜索规划） | **Thinking（仅限 Teacher）** | 推理链提升复杂空间推理 |
| 结构化短输出（bbox JSON、区域排序列表） | **No-Thinking** | 输出格式更稳定，不会被推理链干扰 |
| 闭环导航动作输出 | **No-Thinking** | VLN 实测证明 thinking 导致过早停止、格式溢出 |
| 端侧实时部署 | **锁定 No-Thinking** | 延迟从 2.4s 降至 0.5s，满足实时性约束 |
| Teacher 数据生成 | **按任务切换** | 检测数据用 no-thinking，推理数据用 thinking |

> **两套 benchmark 的矛盾说明"空间关系"能力有两种形态：**
> - MMBench 空间关系：考查"描述性空间推理"，需要文字化推理过程，thinking 链有帮助
> - homeobjects 几何关系：考查"感知性空间判断"，需要直接输出判断，thinking 链反而引入犹豫
